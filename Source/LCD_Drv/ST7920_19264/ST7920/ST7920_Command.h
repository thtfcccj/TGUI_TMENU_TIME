...
//开上半屏
#define		Lcd_OnUpperScreen()	do{set_bit(LCD_E_PORT,LCD_E);}while(0)	//读写使能信号E1，1有效
//开下半屏
//#define		Lcd_OnUnderScreen()	do{PSD8xx_reg.DATAOUT_C |= 0x08;}while(0)	//读写使能信号E2，1有效
#define		Lcd_OnUnderScreen()	Lcd_OnUpperScreen()
//关上半屏
#define		Lcd_OffUpperScreen()	do{clear_bit(LCD_E_PORT,LCD_E);}while(0)	//读写使能信号E1，1有效
//关下半屏
//#define		Lcd_OffUnderScreen()	do{PSD8xx_reg.DATAOUT_C &= ~0x08;}while(0)	//读写使能信号E2，1有效
#define		Lcd_OffUnderScreen()		Lcd_OffUpperScreen()
//选择数据
#define		Lcd_D_Select()   do{set_bit(LCD_RS_PORT,LCD_RS);}while(0)	//数据指令选择信号RS；1为数据操作
//选择指令
#define		Lcd_I_Select()   do{clear_bit(LCD_RS_PORT,LCD_RS);}while(0)	//数据指令选择信号RS；0为写指令或读状态
//读选择信号
#define		Lcd_RD_Select()		do{set_bit(LCD_R_W_PORT,LCD_R_W);}while(0)	//读写选择信号RW；1为读选通
//写选择信号
#define		Lcd_WR_Select()		do{clear_bit(LCD_R_W_PORT,LCD_R_W);}while(0)	//读写选择信号RW；0为写选通
