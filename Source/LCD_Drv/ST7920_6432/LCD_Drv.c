...
#define DelayUs(x) vDelayUs(x)
#define DelayMs(x) vDelayMs(x)
                          ...
                          const char *pBuf)    //需更新字符
{
  ...
  //置字符置起始值
	switch(y){
		...
		case 2: Lcd_UnderWrCommand(SetDDRAMAddr(0x88 + x)); break;	  //0x80-0x8b
		case 3: Lcd_UnderWrCommand(SetDDRAMAddr(0x98 + x)); break;    //0x90-0x9b
  ...
  for(i = 0; i < 16; i++){  //i表示点行
    ...
    if(y < 2){
      ...
      Lcd_UpperWrDataRdy();
  	  ...
  	  Lcd_UnderWrCommand(SetGDRAMAddr(x+ 0x08));	          //水平地址
      Lcd_UnderWrDataRdy();
    }
    //送一点行内连续数据,检查送什么数
    for(j = 0; j < w; j++){
